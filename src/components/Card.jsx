    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (data.meals) {
          ...
          // If no meals are found, set an error
          setRecipe([]);
        ...
        console.error(error);
        setErrors('No recipes found');
        // setRecipe([]); // Clear the recipes on error
      } finally {
        setLoading(false);
      }
    };
    fetchData();
  }, [query, setLoading, setRecipe]);
...
//           // currentItems.length === 0 ? Array.from({ length: 4 }).map((_, index) => <Placeholder key={index} />)
//           //   :
//           :
//           (
//             currentItems.map((recipes) => (
